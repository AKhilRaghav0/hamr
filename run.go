...
// startConfigWatcher sets up a file watcher for the configuration file if
// s.config.configFile is non-empty. It returns a function that stops the
// watcher; the caller should call it when ctx is done.
func (s *Server) startConfigWatcher(ctx context.Context) func() {
	...
	if err != nil {
		s.logger.Error("failed to create config file watcher", "error", err)
		return func() {}
	}
	defer watcher.Close()

	if err := watcher.Add(s.config.configFile); err != nil {
		s.logger.Error("failed to watch config file", "path", s.config.configFile, "error", err)
		return func() {}
	}
...
func (s *Server) runStdio() error {
	...
	stopWatcher := s.startConfigWatcher(ctx)
	defer stopWatcher()
